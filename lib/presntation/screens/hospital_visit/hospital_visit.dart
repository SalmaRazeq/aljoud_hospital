...
class HospitalVisitScreen extends StatefulWidget {
  const HospitalVisitScreen({super.key});
...
class _HospitalVisitScreenState extends State<HospitalVisitScreen> {
                ...
                child: Directionality(
                  ...
                  child: Row(
                    children: [
                      ...
                      InkWell(
                        onTap: () {
                          Navigator.pushNamed(context, RoutesManager.doctorProfile, arguments: doctorData);
                        },
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              doctorData.specialty ?? " لا يحتوي علي تخصص",
                  ...
                  children: [
                    for (var period in ["morning", "evening", "night"])
                      if (visitSlots[selectedDay] != null && (visitSlots[selectedDay]![period]?.isNotEmpty ?? false)) ...[
                        Text(
                          '${loc.translate(period)} ${visitSlots[selectedDay]![period]!.length} ${loc.slots}',
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          final selectedDoctor = DoctorModel(
                            doctorName: doctorData.drName ?? " لا يوجد اسم ",
                            specialty: doctorData.specialty ?? " لا يوجد تخصص",
                            date: _getReadableSelectedDay(selectedDay),
                            time: selectedTime!,
                            price: doctorData.price ?? " لايوجد سعر ",
  ...
  String _getReadableSelectedDay(String key) {
    ...
    switch (key) {
      case "today":
        return '${loc.today}, ${now.day} ${_getMonthName(now.month)}';
      case "tomorrow":
        final date = now.add(const Duration(days: 1));
        return '${loc.tomorrow}, ${date.day} ${_getMonthName(date.month)}';
